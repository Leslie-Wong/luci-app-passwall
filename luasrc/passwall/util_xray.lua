        ...
        inbounds = {
            {
                ...
                streamSettings = {
                    network = node.transport,
                    security = ("1" == node.reality) and "reality" or "none",
					realitySettings = ("1" == node.tls and "1" == node.reality) and {
						show = (node.reality_show == "1") and true or false,
						dest = node.reality_dest,
						xver = tonumber(node.reality_xver),
						serverNames = node.reality_servername,
						privateKey = node.reality_privateKey,
						shortIds = node.reality_shortids or {""},
                        minClientVer = "",
                        maxClientVer = "",
                        maxTimeDiff = 6000,
					} or nil,
                    sniffing = ("1" == node.tls and "1" == node.reality) and {
                        enabled = true,
                        destOverride = {"http", "tls"}
                    } or nil,
                    tlsSettings = ("1" == node.tls and "0" == node.reality) and {
                    ...
                    httpSettings = (node.transport == "h2"  and "0" == node.reality) and {
